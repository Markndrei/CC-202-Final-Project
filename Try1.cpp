...
class bankAccounts // Class
{
    ...
    int accountCash;
    char type;
    string address, phoneNumber, nationality, civilStatus, passcode;
...
public:
    ...
    void changePasscode (string);        //function to change passcode
    ...
    string retpasscode () const;      //function to return phone number
...
void bankAccounts::changePasscode(string passcode)
{
	string passcode, newCode, prevCode;
	int index = 0;
	int attempts = 1+index;
	cout << "You are about to change your Pass Code \n ";
	cout << "Enter your Previous Passcode: ";
	cin.get(prevCode);
		while (prevCode != passcode, i<3)
		{
...
string bankAccounts::retpasscode () const
{
	return passcode;
}
...
void writeAccount()
{
    ...
    ac.createAccount();
    outFile.write(reinterpret_cast<char *>(&ac), sizeof(bankAccounts));
    outFile.close();
}
...
void displayDetails (int n)
{
    ...
    while (inFile.read(reinterpret_cast<char *>(&ac), sizeof(bankAccounts)))
    {
        if (ac.retpasscode() == n)
        {
            ac.showAccount();
...
void modifyAccount(int n)
{
    ...
    {
        File.read(reinterpret_cast<char *>(&ac), sizeof(bankAccounts));
        if (ac.retpasscode() == n)
        {
            ac.showAccount();
            ...
            File.write(reinterpret_cast<char *>(&ac), sizeof(bankAccounts));
            cout << "\n\n\t Record Updated";
            found = true;
        }
    }
    ...
    while (!File.eof() && found == false)
    {
        ...
        if (ac.retpasscode() == n)
        {
            ...
            if (option == 2)
            {
                ...
                cin >> amount;
                int bal = ac.retaccountCash() - amount;
